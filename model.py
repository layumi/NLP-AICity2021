...
class ft_net_EF4(nn.Module):
    def __init__(self, class_num, droprate=0.2, circle=False, noisy=False):
        super().__init__()
        if noisy:
            self.model =EfficientNet.from_pretrained('efficientnet-b4', weights_path = '../EfficientNet-PyTorch-NoisyStudent/pt-models/noisy-student-efficientnet-b4.pth')
        else:
...
class ft_net_EF5(nn.Module):
    def __init__(self, class_num, droprate=0.2, circle=False, noisy=False):
        super().__init__()
        if noisy:
            self.model =EfficientNet.from_pretrained('efficientnet-b5', weights_path = '../EfficientNet-PyTorch-NoisyStudent/pt-models/noisy-student-efficientnet-b5.pth')
        else:
...
class ft_net_EF6(nn.Module):
    def __init__(self, class_num, droprate=0.2, circle=False, noisy=False):
        super().__init__()
        if noisy:
            self.model =EfficientNet.from_pretrained('efficientnet-b6', weights_path = '../EfficientNet-PyTorch-NoisyStudent/pt-models/noisy-student-efficientnet-b6.pth')
        else:
            self.model = EfficientNet.from_pretrained('efficientnet-b6')
        self.model._fc = nn.Sequential()
        self.classifier = ClassBlock(2304, class_num, droprate, return_f = circle)
...
class ft_net_NAS(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False):
        ...
        self.classifier = ClassBlock(4032, class_num, droprate, return_f = circle)
...
class ft_net_DSE(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, pool='avg', circle=False):
        super().__init__()
...
class ft_net_IR(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, circle=False):
        ...
        self.model = model_ft
        # For DenseNet, the feature dim is 2048
        self.classifier = ClassBlock(1536, class_num, droprate, return_f = circle)
...
class PCB(nn.Module):
    def __init__(self, class_num, init_model=None ):
        ...
        model_ft = models.resnet50(pretrained=True)
        self.model = model_ft
        self.part_avgpool1 = nn.AdaptiveAvgPool2d((self.part, 1))
        self.part_maxpool1 = nn.AdaptiveMaxPool2d((self.part, 1))
        self.part_avgpool2 = nn.AdaptiveAvgPool2d((1, self.part))
        self.part_maxpool2 = nn.AdaptiveMaxPool2d((1, self.part))
        ...
        for i in range(2, self.part + 2): # add one original
            ...
            setattr(self, name, ClassBlock(4096, class_num, droprate=0.2, relu=False, bnorm=True, extra_bn=True, num_bottleneck=512))
    ...
    def forward(self, x):
        ...
        predict[2] = self.classifier2(x2.view(x2.size(0), 2*x2.size(1)))
        predict[3] = self.classifier3(x3.view(x3.size(0), 2*x3.size(1)))
