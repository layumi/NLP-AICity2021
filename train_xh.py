...
def compute_loss(model, input_ids, attention_mask, crop, motion, nl_id, crop_id, label, warm):
    ...
    loss_con = 0.0 #F.cross_entropy(sim1, sim_label, ignore_index = -1) + F.cross_entropy(sim2, sim_label, ignore_index = -1)
    loss_cv =  0.0 #F.cross_entropy(predict_class_v, crop_id.cuda(), ignore_index = -1)
    loss_nl =  0.0 #F.cross_entropy(predict_class_l, nl_id.cuda(), ignore_index = -1)
    loss_mt = 0.0
